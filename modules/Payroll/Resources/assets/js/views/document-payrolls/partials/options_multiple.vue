        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tr v-for="(document, index) in document_list" :key="index">
                    <td>{{ document.number_full }}</td>
                    <td>{{ document.response_api_message }}</td>
                    <td>
                        <button type="button" class="btn btn-lg btn-info waves-effect waves-light" @click="clickDownload(document.filename_pdf)">
                            <i class="fa fa-file-pdf"></i>
                        </button>
                    </td>
                    <td>
                        <button type="button" class="btn btn-lg btn-info waves-effect waves-light" @click="clickDownload(document.filename_xml)">
...
<script>
    export default {
        props: ['showDialog', 'recordIds', 'showClose', 'showDownload'],
        data() {
            return {
                titleDialog: 'Nóminas',
                loading: false,
        ...
        methods: {
            ...
            create() {
                this.recordIds.forEach(element => {
                    console.log(element)
                    this.$http.get(`/${this.resource}/record/${element}`).then(response => {
                        this.document_list.push(response.data.data);
                    });
                });
